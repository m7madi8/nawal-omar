...
export const PS_INLINE_SCRIPT = `(function () {
  ...
  ready(function () {
    ...
    form.addEventListener('submit', function (event) {
      ...
      var payload = window.nawalRetreatRequest
      ...
      var submitPromise = payload && window.nawalRetreatRequest
        ? window.nawalRetreatRequest.submit(payload)
        : (function () {
            ...
            return fetch(supabaseUrl + '/rest/v1/' + supabaseTable, {
              method: 'POST',
              headers: {
      ...
      submitPromise
        .then(function () {
          ...
          if (window.nawalThankYou && typeof window.nawalThankYou.show === 'function') {
            window.nawalThankYou.show({ name: data.fullName });
          }
          if (successEl) {
